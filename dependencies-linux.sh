...
test -e `pwd`/installs/android-toolchain || ${ANDROID_NDK_HOME}/build/tools/make_standalone_toolchain.py -v --api=${NDK_TOOLCHAIN_PLATFORM} --arch=${NDK_TOOLCHAIN_ARCHITECTURE} --install-dir=`pwd`/installs/android-toolchain 
popd
