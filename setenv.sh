...
if uname -a | grep -q -i darwin; then
    ...
    export ANDROID_NDK_HOME=`pwd`/installs/android-ndk-${NDK_VERSION}
    export JAVA_HOME=/usr/lib/jvm/java-8-oracle
    export ANDROID_HOME=`pwd`/installs/android-sdk-linux
    export ANDROID_SDK=`pwd`/installs/android-sdk-linux
    export PATH=`pwd`/installs/apache-maven-${MAVEN_VERSION}:`pwd`/installs/gradle-${GRADLE_VERSION}/bin:`pwd`/installs/android-sdk-linux/tools:$PATH
fi

export PATH=$ANDROID_SDK/tools:$ANDROID_SDK/tools/bin:$ANDROID_SDK/platform-tools:${ANDROID_NDK_HOME}:$PATH

export PATH=`pwd`/installs/android-toolchain/bin:$PATH
